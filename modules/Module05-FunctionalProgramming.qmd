...
## What is functional programming?: functionals
...
## Functional vs. imperative programming: imperative
...
## Functional vs. imperative programming: functional
...
## Ok, but why?: loop result

```{r}
do.call(rbind, out)
```

## Ok, but why?: repeat with `lapply()`
...
## Anonymous functions: named equivalent
...
## `*apply()` and friends: common tools
...
## A `tapply()` example: grouped means
...
## A `tapply()` example: grouped means result

```{r}
head(out, 4)
```

## A `tapply()` example: missing values
...
## A `tapply()` example: missing values result
...
## You try it with `lapply()`: solution inventory
...
## You try it with `lapply()`: inventory
