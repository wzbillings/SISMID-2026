...
-   For the first few examples, choose HTML output. HTML renders quickly and
...
-   Use the editor toggle to switch between Visual and Source modes. The
    rendered document is the same either way; the difference is how much raw
    Markdown you see while editing.
...
-   Chunk header: ```` ```{r qcrc-bmi-histogram} ````
-   Chunk option: `#| fig-cap: "BMI distribution in the QCRC sample"`
-   Chunk code: `hist(qcrc_main$bmi)`
